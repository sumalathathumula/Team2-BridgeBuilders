...
Given Admin creates POST Request for the "<Auth>" LMS with "<request body">
When Admin sends HTTPS Request and  request Body with "<endpoint>"
Then Admin receives "<status code>" with "<status message>"

Examples:
|Auth		|	request body	|	endpoint	|	status code		|	status message		|
|Valid		|	Valid			|	Valid		|	201				|	Created				|
|InValid	|	Valid			|	Valid		|	401				|	Unauthorized		|
|Valid		|	Valid			|	Invalid		|	404				|	Not found			|
|Valid		|	Invalid			|	Valid		|	400				|	Bad Request			|
|Valid		|	Missing			|	Valid		|	400				|	Bad Request			|
